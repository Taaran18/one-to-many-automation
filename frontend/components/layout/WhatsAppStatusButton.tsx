...
export default function WhatsAppStatusButton({ compact = false }: { compact?: boolean }) {
  ...
  const fetchStatus = useCallback(async () => {
    try { const d = await apiGet<WAStatus>("/whatsapp/status"); setStatus(d.status); } catch {}
  }, []);
  ...
  useEffect(() => {
    ...
    const iv = setInterval(async () => {
      try {
        ...
        if (d.status === "connected") { setStatus("connected"); setQr(null); clearInterval(iv); }
  ...
  const handleConnect = async () => {
    setConnecting(true);
    try { await apiPost("/whatsapp/connect"); setStatus("qr_pending"); }
  ...
  return (
    <>
      ...
      >
        <span className={`w-2 h-2 rounded-full shrink-0 ${isActive ? "bg-emerald-500" : "bg-red-500 animate-pulse"}`} />
        <span className="truncate">{isActive ? "WhatsApp Connected" : "Connect WhatsApp"}</span>
      </button>
        ...
        <div className="flex flex-col items-center gap-5 py-2">
          {status === "connected" ? (
            <>
              ...
              <p className="font-semibold text-emerald-600 dark:text-emerald-400">WhatsApp Connected!</p>
            </>
          ) : status === "qr_pending" ? (
            <>
              <p className="text-sm text-gray-500 dark:text-gray-400 text-center">Scan with your WhatsApp mobile app</p>
              {qr ? (
            ...
            <>
              ...
              <div className="text-center">
                ...
                <p className="text-sm text-gray-400 dark:text-gray-500">Scan a QR code with your phone to start sending</p>
              </div>
