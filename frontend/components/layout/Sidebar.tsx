...
export default function Sidebar() {
  ...
  const { email, initial } = typeof window !== "undefined" ? getUserInfo() : { email: "", initial: "U" };
  ...
  return (
    <aside className="fixed left-0 top-0 h-full w-64 flex flex-col z-30 bg-white dark:bg-gray-950 border-r border-gray-100 dark:border-gray-800/60">
      ...
      <div className="flex items-center gap-3 px-5 py-5 shrink-0">
        ...
        <div>
          <p className="text-sm font-bold text-gray-900 dark:text-white leading-none">OneToMany</p>
          <p className="text-[11px] text-gray-400 dark:text-gray-500 mt-0.5">WhatsApp Automation</p>
        </div>
      </div>
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-1">
        ...
        {NAV.map((item) => {
          ...
          return (
            ...
            >
              <span className={active ? "text-white" : "text-gray-400 dark:text-gray-500 group-hover:text-gray-600 dark:group-hover:text-gray-300 transition-colors"}>
                {item.icon}
              </span>
