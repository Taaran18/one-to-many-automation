        ...
        <div className="flex flex-col gap-5 py-1">
          ...
          {isConnected ? (
            ...
            <form onSubmit={handleConnect} className="space-y-4">
              ...
              {/* Gmail — Option 1: Google OAuth */}
              {provider === "gmail" && (
                <div className="rounded-2xl border border-gray-100 dark:border-slate-700 p-4 flex flex-col gap-3">
                  <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Option 1 — Sign in with Google</p>
                  <p className="text-xs text-gray-500 dark:text-slate-400">Recommended for work, school, or personal Google accounts. No app password needed.</p>
                  <button
                    type="button"
                    onClick={() => {
                      const token = localStorage.getItem("access_token");
                      window.location.href = `${API_BASE}/email/google/connect?token=${token}`;
                    }}
                    className="w-full h-11 flex items-center justify-center gap-3 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 hover:bg-gray-50 dark:hover:bg-slate-700 rounded-xl text-sm font-semibold text-gray-700 dark:text-slate-200 transition-all"
                  >
              ...
              {/* Divider between the two methods (Gmail only) */}
              {provider === "gmail" && (
                <div className="flex items-center gap-3">
                  <div className="flex-1 h-px bg-gray-100 dark:bg-slate-700" />
                  <span className="text-xs font-semibold text-gray-400 dark:text-slate-500 uppercase tracking-widest">or</span>
                  <div className="flex-1 h-px bg-gray-100 dark:bg-slate-700" />
                </div>
              )}

              {/* Option 2 label (Gmail) or plain fields (other providers) */}
              {provider === "gmail" && (
                <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mt-1">Option 2 — App Password</p>
              )}
