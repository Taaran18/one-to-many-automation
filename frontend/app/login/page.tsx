  ...
  return (
    <div ref={ref} className="relative shrink-0">
      <button
        ...
        className="h-12 flex items-center gap-1.5 px-3 bg-gray-50 border border-gray-200 rounded-2xl text-sm font-semibold text-gray-800 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all hover:bg-gray-100 w-[88px]"
      >
        <span className="text-xl leading-none">{selected.flag}</span>
        <span className="text-gray-600 text-xs">{selected.code}</span>
        <svg
          className={`w-3 h-3 text-gray-400 ml-auto transition-transform duration-200 ${open ? "rotate-180" : ""}`}
      ...
      {open && (
        <div className="absolute top-full left-0 mt-1.5 w-48 bg-white border border-gray-100 rounded-2xl shadow-xl shadow-black/10 z-50 overflow-hidden py-1">
          {COUNTRY_CODES.map((c) => (
            <button
              key={c.code}
              type="button"
              onClick={() => { onChange(c.code); setOpen(false); }}
              className={`w-full flex items-center gap-3 px-3.5 py-2.5 text-sm transition-colors hover:bg-indigo-50 ${value === c.code ? "bg-indigo-50 text-indigo-700 font-semibold" : "text-gray-700"}`}
            >
      ...
      <div className="relative w-full max-w-[460px] bg-white rounded-3xl shadow-2xl shadow-black/40 overflow-hidden">
        ...
        <div className="px-10 py-10">
          ...
          <div className="flex flex-col items-center mb-8">
            ...
            {/* Animated heading */}
            <FadeSlide id={isLogin ? "login-title" : "signup-title"}>
          ...
          {/* Alerts */}
          {(error || success) && (
            <div className={`flex items-start gap-3 px-4 py-3.5 rounded-2xl text-sm font-medium mb-5 ${success ? "bg-emerald-50 text-emerald-700 border border-emerald-200" : "bg-red-50 text-red-600 border border-red-200"}`}>
          ...
          <div className="flex bg-gray-100 p-1 rounded-2xl mb-6 relative">
            {/* Sliding pill */}
            <div
              className="absolute top-1 bottom-1 w-[calc(50%-4px)] bg-white rounded-xl shadow-sm transition-transform duration-300 ease-[cubic-bezier(.4,0,.2,1)]"
              style={{ transform: loginType === "phone" ? "translateX(calc(100% + 8px))" : "translateX(0)" }}
            />
            {(["email", "phone"] as const).map((t) => (
              <button
                ...
                className={`relative flex-1 py-2.5 text-sm font-semibold rounded-xl transition-colors duration-200 z-10 ${loginType === t ? "text-gray-900" : "text-gray-400 hover:text-gray-600"}`}
              >
          ...
          <FadeSlide id={`${isLogin}-${loginType}`}>
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="space-y-2">
                ...
                {loginType === "email" ? (
                  ...
                  <div className="flex gap-2">
                    <FlagSelect value={countryCode} onChange={setCountryCode} />
                    <input
                      type="tel"
                      required
                      value={identifier}
              ...
              <div className="space-y-2">
                ...
                {!isLogin && password.length > 0 && (
                  <div className="pt-1">
                    ...
                    <p className="text-xs text-gray-400">Strength: <span className="font-semibold text-gray-600">{STRENGTH_LABEL[strength]}</span></p>
                  </div>
                )}
              </div>
              ...
              <button
                type="submit"
                disabled={isLoading || (!isLogin && strength < 4)}
          ...
          <div className="space-y-3">
            <button onClick={() => (window.location.href = "http://localhost:8000/auth/google/login")} type="button" className="w-full h-12 flex items-center justify-center gap-3 bg-white border border-gray-200 hover:bg-gray-50 hover:border-gray-300 rounded-2xl text-sm font-semibold text-gray-700 transition-all">
            ...
            <div className="grid grid-cols-2 gap-3">
              <button onClick={() => (window.location.href = "http://localhost:8000/auth/apple/login")} type="button" className="h-12 flex items-center justify-center gap-2.5 bg-white border border-gray-200 hover:bg-gray-50 hover:border-gray-300 rounded-2xl text-sm font-semibold text-gray-700 transition-all">
              ...
              <button onClick={() => (window.location.href = "http://localhost:8000/auth/microsoft/login")} type="button" className="h-12 flex items-center justify-center gap-2.5 bg-white border border-gray-200 hover:bg-gray-50 hover:border-gray-300 rounded-2xl text-sm font-semibold text-gray-700 transition-all">
