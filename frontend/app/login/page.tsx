  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (loginType === "phone") {
      ...
      const ok = Array.isArray(req)
        ? req.includes(identifier.length)
        : identifier.length === req;
      if (!ok) {
        setError(`Phone number length is invalid for ${countryCode}`);
        return;
      }
    } else {
    ...
    setIsLoading(true);
    const username =
      loginType === "phone" ? `${countryCode}${identifier}` : identifier;
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-900 px-4 py-12 relative overflow-hidden">
      ...
      <div className="relative w-full max-w-[460px] bg-white rounded-3xl shadow-2xl shadow-black/40 overflow-hidden">
        ...
        <div className="px-10 py-10">
          ...
          {(error || success) && (
            <div
              className={`flex items-start gap-3 px-4 py-3.5 rounded-2xl text-sm font-medium mb-5 ${success ? "bg-emerald-50 text-emerald-700 border border-emerald-200" : "bg-red-50 text-red-600 border border-red-200"}`}
            >
              <svg
                  ...
                  <div className="pt-1">
                    <div className="flex gap-1.5 mb-1.5">
                      {[1, 2, 3, 4].map((l) => (
                        <div
                          key={l}
                          className={`h-1.5 flex-1 rounded-full transition-all duration-300 ${strength >= l ? STRENGTH_COLOR[strength] : "bg-gray-200"}`}
                        />
                      ))}
                    </div>
