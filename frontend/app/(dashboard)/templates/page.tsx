...
export default function TemplatesPage() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      setTemplates(await apiGet<Template[]>("/templates/"));
    } catch {
    } finally {
      setLoading(false);
    }
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      editing
        ? await apiPut(`/templates/${editing.id}`, form)
      ...
      {loading ? (
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800">
          <EmptyState
            ...
            description="Create templates with dynamic variables like {{name}} and {{phone}} for personalized messaging."
            action={
              <button onClick={openNew} className={BTN_P}>
                Create Template
              </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
          {templates.map((t) => (
            ...
            >
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400 whitespace-pre-wrap line-clamp-4 leading-relaxed">
                {t.body}
              </p>
              <div className="pt-3 border-t border-gray-100 dark:border-gray-800">
                <p className="text-xs font-semibold text-gray-400 dark:text-gray-600 mb-1.5">
                  Preview
                </p>
                <p className="text-xs text-gray-500 dark:text-gray-400 whitespace-pre-wrap line-clamp-3 leading-relaxed bg-gray-50 dark:bg-gray-800 rounded-lg p-2.5">
                  {prev(t.body)}
                </p>
              </div>
            </div>
