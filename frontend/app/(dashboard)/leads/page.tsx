...
export default function LeadsPage() {
  ...
  const [form, setForm] = useState({ name: "", phone_no: "", email: "", tags: "", status: "prospect" });
  ...
  const load = async (q = search, s = filter) => {
    setLoading(true);
    try {
      let path = `/leads/?limit=200${q ? `&search=${encodeURIComponent(q)}` : ""}${s ? `&status=${s}` : ""}`;
      const [l, g] = await Promise.all([apiGet<Lead[]>(path), apiGet<LeadGroup[]>("/leads/groups/all")]);
      setLeads(l); setGroups(g);
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try { await apiPost("/leads/", form); setAddOpen(false); setForm({ name: "", phone_no: "", email: "", tags: "", status: "prospect" }); load(); }
  ...
  const handleCsv = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const mapped = rows.map(r => ({ name: r.name || r.full_name || "Unknown", phone_no: r.phone || r.phone_no || r.mobile || "", email: r.email, tags: r.tags, status: r.status === "customer" ? "customer" : "prospect" })).filter(r => r.phone_no);
    if (!mapped.length) return alert("No valid rows — check CSV has a 'phone' column");
    setSaving(true);
    try { await apiPost("/leads/import", mapped); load(); alert(`${mapped.length} leads imported!`); }
    catch (err: any) { alert(err.message); } finally { setSaving(false); if (fileRef.current) fileRef.current.value = ""; }
  };

  const handleGroup = async (e: React.FormEvent) => {
    e.preventDefault(); setSaving(true);
    try { await apiPost("/leads/groups", groupForm); setGroupOpen(false); setGroupForm({ name: "", description: "" }); load(); }
    catch (err: any) { alert(err.message); } finally { setSaving(false); }
  };

  const handleDelete = async (id: number) => {
    if (!confirm("Delete this lead?")) return;
    try { await apiDelete(`/leads/${id}`); load(); } catch (err: any) { alert(err.message); }
  };
    ...
    <div className="space-y-6">
      ...
      {groups.length > 0 && (
        <div className="flex gap-2 flex-wrap">
          {groups.map(g => (
            <div key={g.id} className="flex items-center gap-2 px-3 py-1.5 rounded-xl text-xs font-semibold bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-400">
              <span className="w-1.5 h-1.5 rounded-full bg-indigo-400" />{g.name}
                  ...
                  <td className="px-5 py-4 hidden lg:table-cell">
                    {lead.tags ? (
                      <div className="flex flex-wrap gap-1">
                        {lead.tags.split(",").map(t => <span key={t} className="text-xs px-2 py-0.5 rounded-full bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400">{t.trim()}</span>)}
                      </div>
                    ) : <span className="text-gray-400">—</span>}
                  </td>
                  <td className="px-5 py-4"><Badge label={lead.status} /></td>
                  <td className="px-5 py-4 text-right">
                    <button onClick={() => handleDelete(lead.id)} className="text-xs font-semibold px-3 py-1.5 rounded-lg bg-red-50 dark:bg-red-900/20 text-red-500 dark:text-red-400 hover:bg-red-100 dark:hover:bg-red-900/40 transition-all">Delete</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <Modal open={addOpen} onClose={() => setAddOpen(false)} title="Add New Lead">
        <form onSubmit={handleAdd} className="space-y-4">
          {[{l:"Name *",k:"name",p:"Full name",t:"text"},{l:"Phone *",k:"phone_no",p:"+919876543210",t:"text"},{l:"Email",k:"email",p:"email@example.com",t:"email"},{l:"Tags",k:"tags",p:"VIP, Mumbai",t:"text"}].map(f => (
            <div key={f.k}><label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 mb-1.5">{f.l}</label>
              <input type={f.t} required={f.l.includes("*")} value={(form as any)[f.k]} onChange={e => setForm({...form,[f.k]:e.target.value})} placeholder={f.p} className={INPUT} /></div>
          ))}
          <div><label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 mb-1.5">Status</label>
      ...
      <Modal open={groupOpen} onClose={() => setGroupOpen(false)} title="New Lead Group">
        <form onSubmit={handleGroup} className="space-y-4">
          <div><label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 mb-1.5">Group Name *</label>
            <input required value={groupForm.name} onChange={e => setGroupForm({...groupForm,name:e.target.value})} placeholder="e.g. Mumbai Clients" className={INPUT} /></div>
          <div><label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 mb-1.5">Description</label>
            <textarea value={groupForm.description} onChange={e => setGroupForm({...groupForm,description:e.target.value})} placeholder="Optional description" rows={3} className={`${INPUT} resize-none`} /></div>
          <div className="flex gap-3 pt-1">
