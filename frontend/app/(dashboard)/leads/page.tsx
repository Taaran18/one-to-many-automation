  ...
  const handleCsv = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const mapped = rows
      .map((r) => ({
        ...
        email: r.email,
        tags: r.tags,
        status: r.status === "customer" ? "customer" : "prospect",
      }))
      .filter((r) => r.phone_no);
    if (!mapped.length)
    ...
    <div className="space-y-6">
      ...
      {groups.length > 0 && (
        <div className="flex gap-2 flex-wrap">
          {groups.map((g) => (
            <div
              key={g.id}
              className="flex items-center gap-2 px-3 py-1.5 rounded-xl text-xs font-semibold bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-400"
            >
      ...
      <div className="flex gap-3 flex-wrap">
        ...
        <select
          ...
          className="bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl px-3.5 py-2.5 text-sm text-gray-700 dark:text-gray-300 outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-500 transition-all"
        >
      ...
      >
        <form onSubmit={handleAdd} className="space-y-4">
          {[
            { l: "Name *", k: "name", p: "Full name", t: "text" },
            { l: "Phone *", k: "phone_no", p: "+919876543210", t: "text" },
            { l: "Email", k: "email", p: "email@example.com", t: "email" },
            { l: "Tags", k: "tags", p: "VIP, Mumbai", t: "text" },
          ].map((f) => (
            <div key={f.k}>
