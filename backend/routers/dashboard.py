...
def get_stats(
    ...
    total_leads = (
        db.query(func.count(models.Lead.id)).filter(models.Lead.user_id == uid).scalar()
        or 0
    )

    # Leads that have been sent at least one message
    leads_touched = (
        db.query(func.count(func.distinct(models.MessageLog.lead_id)))
        .join(models.Campaign, models.MessageLog.campaign_id == models.Campaign.id)
        .filter(
    ...
    total_templates = (
        ...
        .filter(models.Template.user_id == uid)
        .scalar()
        or 0
    )
...
def get_schedule(
    ...
    campaigns = (
        ...
        .order_by(models.Campaign.scheduled_at)
        .limit(10)
        .all()
    )
...
def get_chart(
    ...
    rows = (
        ...
        .group_by(func.date(models.MessageLog.sent_at))
        .order_by("date")
        .all()
    )
